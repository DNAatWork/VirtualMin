...
{
return "Custom WordPress";
}

sub script_custompress_uses
{
return ( "php" );
}

sub script_custompress_longdesc
{
return "A semantic personal publishing platform with a focus on aesthetics, web standards, and usability.";
}

# script_custompress_versions()
sub script_custompress_versions
{
return ( "3.8.1" );
}

sub script_custompress_category
{
return "Blog";
}

sub script_custompress_php_vers
{
local ($d, $ver) = @_;
if ($ver >= 3.2) {
	return ( 5 );
...
sub script_custompress_php_modules
{
return ("mysql", "gd");
}

sub script_custompress_php_optional_modules
{
return ("curl");
}

sub script_custompress_dbs
{
return ("mysql");
}

# script_custompress_depends(&domain, version)
sub script_custompress_depends
{
...
# script_custompress_params(&domain, version, &upgrade-info)
# Returns HTML for table rows for options for installing Wordpress
sub script_custompress_params
{
...
# script_custompress_parse(&domain, version, &in, &upgrade-info)
# Returns either a hash ref of parsed options, or an error string
sub script_custompress_parse
{
...
# script_custompress_check(&domain, version, &opts, &upgrade-info)
# Returns an error message if a required option is missing or invalid
sub script_custompress_check
{
...
sub script_custompress_commands
{
return ("unzip");
}

# script_custompress_install(&domain, version, &opts, &files, &upgrade-info)
# Actually installs WordPress, and returns either 1 and an informational
# message, or 0 and an error
sub script_custompress_install
{
...
# script_custompress_realversion(&domain, &opts)
# Returns the real version number of some script install, or undef if unknown
sub script_custompress_realversion
{
...
# script_custompress_latest(version)
# Returns a URL and regular expression or callback func to get the version
sub script_custompress_latest
{
local ($ver) = @_;
